...
def set_task_status(week_abs: int, person: str, chore: str, status: TaskStatus) -> None:
    ...
    # Read previous status before overwriting so we can avoid double-counting
    # stats when a task is toggled (done → pending → done).
    prev_status: TaskStatus = "pending"
    raw = r.get(key)
    if raw is not None:
        try:
            prev_status = json.loads(raw).get("status", "pending")
        except (json.JSONDecodeError, TypeError):
            pass

    r.set(key, json.dumps({"status": status, "marked_at": now.isoformat()}))
    r.zadd("weeks_seen", {str(week_abs): week_abs})

    # Only update stats if the terminal state has actually changed.
    if status in ("done", "skipped") and status != prev_status:
        _update_stats(r, person, week_abs, status, now)


def _update_stats(
    r: Redis, person: str, week_abs: int, status: TaskStatus, marked_at: datetime
) -> None:
    ...
    if status == "done":
        ...
        if _is_on_time(week_abs, marked_at, HOUSE_START_DATE):
            cur["done_on_time"] += 1
        else:
            cur["done_late"] += 1
        if new_week:
...
def get_week_statuses(
    week_abs: int, schedule: dict[str, list[str]]
