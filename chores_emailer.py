...
def send_reminders(mode: str) -> None:
    ...
    mode="monday"  — kick-off email (email_monday.html); also seeds the DB.
    mode="sunday"  — check-in email (email_sunday.html); subject personalised
                     by current completion status.
    """
    if mode not in ("monday", "sunday"):
        raise ValueError(f"mode must be 'monday' or 'sunday', got {mode!r}")
